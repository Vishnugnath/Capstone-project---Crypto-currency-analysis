...
-- 17. CTE
-- Q25: Calculate the daily average closing price across all three cryptocurrencies in the crypto_data view, and then select records where this average exceeds $20,000.

WITH Daily_Avg_Close AS (
    SELECT 
        Date,
        (Bitcoin_Close + Ethereum_Close + Binance_Close) / 3 AS Avg_Close
    FROM crypto_data
)

-- Select dates where the average closing price exceeds $20,000
SELECT Date, Avg_Close
FROM Daily_Avg_Close
WHERE Avg_Close > 20000;
